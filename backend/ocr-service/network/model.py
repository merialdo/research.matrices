...
class HTRModel:
    ...
    def predict(self,
        ...
        #out = self.model(inputs=x, training=False)
        ...
        paths = []
        for p in out:
            seq, path = beam_search(p, dtgen.tokenizer.chars + "§", beam_size=10, beam_cut_threshold=0.01)
            predicts.append(seq.replace("¶", "").replace("¤", "").replace("§", ""))
            paths.append(path)
        '''

        predicts, probabilities = ctc_decodeTF(out)
